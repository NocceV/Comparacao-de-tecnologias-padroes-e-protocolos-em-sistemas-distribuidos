...
URL_REST = "http://localhost:8012/messages/create"
URL_GRAPHQL = "http://localhost:8011/graphql"
URL_SOAP = "http://localhost:8016/soap/messages"
URL_WEBHOOK = "http://localhost:8004/messages"
URL_WS = "ws://localhost:8015/ws/messages"
URL_SSE = "http://localhost:8017/messages/create"
GRPC_ADDR = "localhost:50054"

SOAP_HEADERS = {"Content-Type": "text/xml", "SOAPAction": "CreateMessage"}


def req_rest(index: int) -> tuple[float, bool]:
    payload = {"user": f"User{index}", "content": f"Msg REST {RUN_ID}_{index}"}
    start = time.perf_counter()
    r = requests.post(URL_REST, json=payload, timeout=5)
    elapsed = (time.perf_counter() - start) * 1000
    return elapsed, r.status_code == 200


def req_graphql(index: int) -> tuple[float, bool]:
    query = f"""query {{ message(id: {index}) {{ id user content timestamp }} }}"""
    start = time.perf_counter()
...
<soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">
  <soap:Body>
    <CreateMessage>
      ...
      <content>Msg SOAP {RUN_ID}_{index}</content>
    </CreateMessage>
  </soap:Body>
</soap:Envelope>"""
    start = time.perf_counter()
...
def req_webhook(index: int) -> tuple[float, bool]:
    payload = {"sender": f"User{index}", "content": f"Msg Webhook {RUN_ID}_{index}"}
    start = time.perf_counter()
...
def req_sse(index: int) -> tuple[float, bool]:
    payload = {"user": f"User{index}", "content": f"Msg SSE {RUN_ID}_{index}"}
    start = time.perf_counter()
    r = requests.post(URL_SSE, json=payload, timeout=5)
    elapsed = (time.perf_counter() - start) * 1000
    return elapsed, r.status_code == 200
...
def req_websocket(index: int) -> tuple[float, bool]:
    ...
    async def _send():
        async with websockets.connect(URL_WS, close_timeout=2) as ws:
            payload = {"action": "send_message", "data": {"user": f"User{index}", "content": f"Msg WS {RUN_ID}_{index}"}}
            start = time.perf_counter()
            await ws.send(json.dumps(payload))
            msg = json.loads(await ws.recv())
            elapsed = (time.perf_counter() - start) * 1000
            ok = msg.get("event") == "message_created"
            return elapsed, ok
...
def req_grpc(index: int) -> tuple[float, bool]:
    ...
    if _grpc_stub is None:
        ...
        _grpc_stub = message_pb2_grpc.MessageServiceStub(_grpc_channel)

    start = time.perf_counter()
    try:
        req = message_pb2.GetMessageRequest(id=index)
        _grpc_stub.GetMessage(req, timeout=5)
        elapsed = (time.perf_counter() - start) * 1000
        return elapsed, True
    except Exception:
        elapsed = (time.perf_counter() - start) * 1000
...
def run_random_stress_test(total_requests: int = 500, selected_protocols: Optional[List[str]] = None, output_file: Optional[str] = None):
    ...
    print("====================================================================")
    print("🎲 TESTE DE STRESS ALEATÓRIO (MULTI-PROTOCOLO) — MESSAGE SERVICE")
    print("====================================================================")
    ...
    print("\n" + "=" * 80)
    print("📊 RELATÓRIO COMPARATIVO POR PROTOCOLO (ALEATÓRIO - MESSAGE)")
    print("=" * 80)
...
def main():
    parser = argparse.ArgumentParser(description="Teste de Stress Aleatório Multi-Protocolo (Message Service)")
