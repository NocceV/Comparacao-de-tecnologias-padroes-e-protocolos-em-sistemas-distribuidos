    ...
    for _ in range(10):
        try:
            ...
            if r.status_code == 200:
                print("✅ Serviço Message Webhook disponível!")
                return
        except Exception:
            time.sleep(2)
    pytest.fail("❌ Serviço Message Webhook não iniciou a tempo.")
...
def test_create_message_and_receive_webhook():
    """Cria uma mensagem e verifica se o webhook é recebido."""
    global last_webhook_payload
    message_data = {"sender": "Vitor", "content": "Mensagem de teste via Webhook"}

    resp = requests.post(MESSAGES_URL, json=message_data)
    assert resp.status_code == 200
    message = resp.json()
    assert message["sender"] == "Vitor"
    assert "Mensagem de teste" in message["content"]
    ...
    assert last_webhook_payload["event"] == "message_created"
    data = last_webhook_payload["data"]
    assert data["sender"] == "Vitor"
    assert "Mensagem de teste" in data["content"]
    print("📩 Webhook de mensagem recebido com sucesso:", last_webhook_payload)
